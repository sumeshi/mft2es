...
def create_timeline_record(
    ...
    return {
        ...
        "attribute": {
            ...
            "data": {k: v for k, v in attr_data.items() if k not in MACB_MAPPING.values()},
        },
...
def process_standard_by_chunk(records: List[str], rows: List[bytes]) -> List[dict]:
    ...
    return [
        format_standard_record(record, filename) for record, filename in zip(record_list, filename_list)
    ]
...
class Mft2es(SafeMultiprocessingMixin):
    ...
    def gen_timeline_records(self, multiprocess: bool, chunk_size: int, timeline_mode: bool = False) -> Generator:
        ...
        if multiprocess:
            ...
            if timeline_mode:
                with ctx.Pool(self.get_cpu_count()) as pool:
                    results = pool.starmap_async(process_timeline_by_chunk, zip(generate_chunks(chunk_size, self.parser.entries_json()), generate_chunks(chunk_size, self.csvparser.entries_csv())))
                    yield list(chain.from_iterable(results.get(timeout=None)))
            else:
                with ctx.Pool(self.get_cpu_count()) as pool:
                    results = pool.starmap_async(process_standard_by_chunk, zip(generate_chunks(chunk_size, self.parser.entries_json()), generate_chunks(chunk_size, self.csvparser.entries_csv())))
                    yield list(chain.from_iterable(results.get(timeout=None)))
        else:
